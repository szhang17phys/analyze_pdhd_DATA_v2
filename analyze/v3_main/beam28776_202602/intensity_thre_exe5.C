...
void processSingleFile(const char* inputFileName,
                       const double thre_summed,
                       const char* muonOut,
                       const char* michelOut,
                       const char* thirdOut) {  
    ...
    std::ofstream muonFile(muonOut, std::ios::app);
    std::ofstream michelFile(michelOut, std::ios::app);
    std::ofstream thirdFile(thirdOut, std::ios::app);
...
void processDirectory(const char* directoryName,
    ...
    for (size_t counter = 0; counter < fileList.size(); ++counter) {
        ...
        processSingleFile(fullPath.Data(), thre, muonOut, michelOut, thirdOut);
    }
}
...
//"Part I"----------------------------
void intensity_thre_exe5(const char* inputDirectory,
                    double thre,
                    const char* muonOut,
                    const char* michelOut,
                    const char* thirdOut) {
    ...
    std::ofstream clear1(muonOut);
    std::ofstream clear2(michelOut);
    std::ofstream clear3(thirdOut);
    clear1.close();
    clear2.close();   
    clear3.close();

    processDirectory(inputDirectory, thre, muonOut, michelOut, thirdOut);
}
