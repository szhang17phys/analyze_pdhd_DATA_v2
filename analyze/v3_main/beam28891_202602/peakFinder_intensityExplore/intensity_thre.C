...
void processSingleFile(const char* inputFileName, const double thre_summed) {  
    ...
    std::ofstream muonFile("muon_total_202602.txt", std::ios::app);
    std::ofstream michelFile("michel_total_202602.txt", std::ios::app);
    std::ofstream thirdFile("third_total_202602.txt", std::ios::app);
...
void intensity_thre() {
    ...
    std::ofstream clear1("muon_total_202602.txt");
    std::ofstream clear2("michel_total_202602.txt");
    std::ofstream clear3("third_total_202602.txt");  // NEW
    clear1.close();
    clear2.close();   
    clear3.close();

    const char* inputDirectory = "/Volumes/ssd_zhang/thesis_michel/server_processing/t0_rootFiles/beam28891_new202602/wvf_applyCut_removeBeam";
