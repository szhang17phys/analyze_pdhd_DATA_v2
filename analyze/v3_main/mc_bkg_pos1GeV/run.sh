...
# Configurations:
runFolder="bkg_pos1GeV"  # <-- change this for each dataset          
runID="20250614"                    # <-- full official MC, runID is shown at event_wvf_extract
...
# ===== Run merging script =====
python wvf_coin_merged_exe1.py \
    --input_dir "${input_dir}" \
...
python extract_trueInfo_exe2p.py \
...
python wvf_coin_applyCut_exe3.py \
...
python extract_tpc_info_exe6.py \
    --mcTruth_file1 "${mcTruth_file1}" \
    --mcTruth_file2 "${mcTruth_file2}" \
    --mcTruth_file3 "${mcTruth_file3}" \
    --muon_txt "${muon_txt}" \
