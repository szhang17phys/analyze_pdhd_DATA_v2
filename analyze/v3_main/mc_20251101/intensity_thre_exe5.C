...
std::string DATE = "00000000";  // default value, can be overridden
//==============================================================
...
void processSingleFile(const char* inputFileName, const double thre_summed) {  
    ...
    std::ofstream muonFile(("muon_total_" + DATE + ".txt").c_str(), std::ios::app);
    std::ofstream michelFile(("michel_total_" + DATE + ".txt").c_str(), std::ios::app);
    std::ofstream thirdFile(("third_total_" + DATE + ".txt").c_str(), std::ios::app);
...
// ===================== Main function ===========================
void intensity_thre_exe5_tmp(const char* dateArg, const char* inputDir)
{
    gROOT->cd();

    DATE = dateArg;  // update global

    // Clear output files before starting
    std::ofstream clear1(("muon_total_" + DATE + ".txt").c_str());
    std::ofstream clear2(("michel_total_" + DATE + ".txt").c_str());
    std::ofstream clear3(("third_total_" + DATE + ".txt").c_str());
    clear1.close();
    clear2.close();
    clear3.close();

    // Use directory passed from shell
    std::string inputDirectory = inputDir;
    std::cout << "[INFO] Using input directory: " << inputDirectory << std::endl;

    processDirectory(inputDirectory.c_str(), 0.3); // Threshold for summed wvf
}


// ----------------------------------------------------------------------
// New main function (wrapper)--------
// ----------------------------------------------------------------------
void intensity_thre_exe5(const char* dateArg = "00000000", const char* inputDir = "")
{
    intensity_thre_exe5_tmp(dateArg, inputDir);
}
