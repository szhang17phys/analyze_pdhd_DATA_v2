...
void processSingleFile(const char* inputFileName, double heightThreshold) {  
    ...
    while ((key = (TKey*)nextKey())) { // process TH1D one by one ----------

        TObject* obj = key->ReadObj();
        if (!obj) {
            std::cerr << "[Warning] Null object detected in " << inputFileName << std::endl;
            continue;
        }
        if (!obj->InheritsFrom("TH1D")) {
            delete obj; // Cleanup for non-TH1D objects like TCanvas
            continue;
        }
        ...
        if (histName == "total") {
            ...
            if (topTotalPeaks.size() == 1) {
                std::cout << "[Warning] Only one peak found in 'total'. Marking Michel peak as invalid." << std::endl;
                topTotalPeaks.push_back(1);
            }
        }
    ...
    //Suggested by ChatGPT, to solve memory issue, 20250614---
    delete inputFile;
    //---------------------------------------------------------------------------------    
    ...
    if (nMerged == 0) {
        ...
        // nMerged >= 3: threshold logic applies
        size_t limit = 3;
...
void processDirectory(const char* directoryName, double heightThreshold) {
    ...
    for (size_t counter = 0; counter < fileList.size(); ++counter) {
        ...
        //Zombie file check; suggested by ChatGPT, 20250627---
        TFile* checkFile = TFile::Open(fullPath);
        if (!checkFile || checkFile->IsZombie()) {
            std::cerr << "[Warning] Skipping corrupted or unreadable file: " << fullPath << std::endl;
            delete checkFile;
            continue;
        }
        checkFile->Close();
