...
void processSingleFile(const char* inputFileName, double heightThreshold) {  
    ...
    while ((key = (TKey*)nextKey())) { // process TH1D one by one ----------
        ...
        if (!obj || !obj->InheritsFrom("TH1D")) continue;
...
void processDirectory(const char* directoryName, double heightThreshold) {
    ...
    for (size_t counter = 0; counter < fileList.size(); ++counter) {
        ...
        if (counter == 1209) {
            std::cout << "[DEBUG] File #1209 is: " << fullPath << std::endl;
            std::cout.flush();  // Ensure this gets printed even if crash follows
        }
...
void michel_peakFinder_new20250602() {
    ...
    const char* inputDirectory = "/Users/shuaixiangzhang/Work/current/FNAL_Work2024/michel_e/t0_tagging/pdhd_DATA_v2/t0_rootFiles/data_28891_FullRun/decon_wvf_coincidence_applyCut";
