...
x_path  = "posX_extract_20250920.txt"
y_path  = "posY_extract_20250920.txt"
z_path  = "posZ_extract_20250920.txt"
MS_path = "score_extract_20250920.txt"
MH_path = "hits_extract_20250920.txt"

# Open output files in write mode
with open(x_path, "w") as x_out, open(y_path, "w") as y_out, open(z_path, "w") as z_out, \
     open(MS_path, "w") as ms_out, open(MH_path, "w") as mh_out:
    ...
    #================================================================
    # Process each line in muon_time_20250920.txt
    #================================================================
    with open("./muon_time_20250920.txt", "r") as f:
        for line in f:
            ...
            for i in range(len(filtered_lines)):
                ...
                if "Run:" in current_line and "Event:" in current_line and "TrackID:" in current_line:
                    ...
                    if event == event_target and track == track_target:
                        # --------------------------------------
                        # Extract Michel score and hits (from the line above)
                        # --------------------------------------
                        if i - 1 >= 0:
                            prev_line = filtered_lines[i - 1].strip()
                            ms_match = re.search(
                                r"Michel score:\s*([\d\.Ee+-]+),\s*Michel hits:\s*(\d+)", prev_line
                            )
                            if ms_match:
                                ms_out.write(ms_match.group(1) + "\n")
                                mh_out.write(ms_match.group(2) + "\n")
