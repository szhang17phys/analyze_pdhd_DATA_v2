      ...
      for (size_t ci = 0; ci < candidateIndicesForEvent.size(); ci++) {
        ...
        for (size_t i = 0; i < pandorat0->size(); ++i) {
          if ((*trkid)[i] == candTrack) {
            ...
            TString outputFileName = Form("/exp/dune/data/users/szh2/running_results/MC_PDHD_list/event_wvf_extract/new202508/window_Pdot00_Pdot01/new20250809/extracted_files/mcSingleEvent_run%d_event%d_trackID%d.root", run, event, (*trkid)[i]);
            //====================================================
