...
//        double lifetime_end_us = (parent->EndT() - parent->T()) * 1e-3; // official: muon birth → muon end
        ...
        std::cout << "Michel (PDG = " << part.PdgCode() << ") (trackID = " << part.TrackId() << "): Kinetic = "
            ...
            << "\n  Mu(x0,y0,z0,t0) = (" << parent->Vx() << ", " << parent->Vy() << ", " << parent->Vz() << ", " << parent->T() << " ns)"
            << "\n  Mu(x1,y1,z1,t1) = (" << parent->EndX() << ", " << parent->EndY() << ", " << parent->EndZ() << ", " << parent->EndT() << " ns)"
            << "\n---------------" << std::endl;
    }
    ...
    //======================================================================
    if (iLogLevel >= kInfos) std::cout << "\nLOOPING over " << vp_trk.size() << " tracks..." << std::endl;
    ...
    for (art::Ptr<recob::Track> const& p_trk : vp_trk) {
        ...
        // Match is based on which MCParticle contributed the most ionization energy
        // to the hits (inherent space constraint) used to reconstruct the track (via BackTracker IDEs)
        ...
        if (mcp->EndProcess() == "muMinusCaptureAtRest") {
            if (v_hit_michel.size()) {//If we observe michel, capture here is not true capture---
                n_cme_wh++;
