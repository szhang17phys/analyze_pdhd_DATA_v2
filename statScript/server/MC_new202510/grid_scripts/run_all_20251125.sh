...
# ---------------------------------
# Detsim Stage 1 & Stage 2 combined
# ---------------------------------
DETSIM_FCL="${INPUT_TAR_DIR_LOCAL}/work/my_pdhd_production/scripts/standard_detsim_protodunehd_combined.fcl"
if [ ! -f "$DETSIM_FCL" ]; then
    echo "ERROR: $DETSIM_FCL not found!"
    exit 88
fi

lar -c "$DETSIM_FCL" -s cosmic_g4_${logical_jobid}_${timestamp}.root -o cosmic_detsim_${logical_jobid}_${timestamp}.root

if [ $? -ne 0 ]; then
    echo "Detsim Stage 1&2 (combined) failed"
    exit 18
fi

echo "Detsim Stage 1&2 (combined) completed. Removing input file from G4 stage..."
#ls -lh
...
lar -c "$RECO_FCL" -s cosmic_detsim_${logical_jobid}_${timestamp}.root -o cosmic_reco_${logical_jobid}_eventID${FIRSTEVENT}_${timestamp}.root
if [ $? -ne 0 ]; then
...
rm -f cosmic_detsim_${logical_jobid}_${timestamp}.root
