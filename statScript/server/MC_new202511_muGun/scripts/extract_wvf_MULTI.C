...
//==========Only change this each time!=========================
const std::string DATE = "20251060"; 
//==============================================================


void extract_wvf_MULTI(const char* eventFilePath = "eventID_filtered_sorted.txt",
                       ...
                       const char* opchsFilePath = "opchs_filtered_sorted.txt")
{
    ...
    const std::string outDir = "../event_wvf_extract_" + DATE;
    ...
    try {
        if (!fs::exists(outDir)) {
            ...
            std::cout << "[INFO] Created output directory: " << outDir << std::endl;
        } else {
    ...
    const double dt_min_ms = 0.05;
    const double dt_max_ms = 0.08;
    //==============================================================
